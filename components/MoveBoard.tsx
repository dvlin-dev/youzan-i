...
import { submitMove, reviewDoc, rejectDoc } from "@/lib/actions";
...
export function MoveBoard({ skus, pending }: { skus: SkuRow[]; pending: Pend[] }) {
  ...
  const sizes = [...new Set(cur.map((s) => s.size))].sort((a, b) => SIZE_ORDER.indexOf(a) - SIZE_ORDER.indexOf(b));
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 18, alignItems: "start" }}>
      <div className="card pad">
        <div className="row" style={{ marginBottom: 18 }}>
          <div className="roles" style={{ background: "var(--surface-2)", width: 180 }}>
            <button className="role-btn" style={type === "IN" ? { background: "var(--surface)", color: "var(--ink)" } : {}} onClick={() => setType("IN")}>
              入库
            </button>
            <button className="role-btn" style={type === "OUT" ? { background: "var(--surface)", color: "var(--ink)" } : {}} onClick={() => setType("OUT")}>
              出库
            </button>
          </div>
          <select className="input" style={{ maxWidth: 300, marginLeft: 6 }} value={style} onChange={(e) => setStyle(e.target.value)}>
            {styles.map(([no, name]) => (
        ...
        <div className="field">
          <label>
            按 颜色 × 尺码 矩阵录入数量 <span className="dim" style={{ fontWeight: 400 }}>像 Excel 一样快</span>
          </label>
                ...
                {colors.map((c) => (
                  <tr key={c}>
                    ...
                    {sizes.map((sz) => {
                      ...
                      if (!s)
                        return (
                          <td key={sz} style={{ textAlign: "center" }} className="dim">
                            —
                          </td>
                        );
                      return (
                        <td key={sz} style={{ textAlign: "center" }}>
                          <input
                            ...
                            value={qty[s.skuCode] ?? ""}
                            placeholder="0"
                            onChange={(e) => setQty((q) => ({ ...q, [s.skuCode]: e.target.value }))}
                          />
                          <div className="dim" style={{ fontSize: 10.5, marginTop: 2 }}>现 {s.qty}</div>
                        </td>
                      );
                    })}
                  </tr>
        ...
        <div className="between" style={{ borderTop: "1px solid var(--border)", paddingTop: 14, marginTop: 4 }}>
          <div>
            合计 <b className="tnum" style={{ fontSize: 18, color: "var(--ink)" }}>{total}</b> 件 ·{" "}
            <span className="dim">{type === "IN" ? "入库增加" : "出库减少"}库存</span>
          </div>
      ...
      <div className="card pad">
        ...
        {pending.map((p) => (
          <div className="alert-row" key={p.doc} style={{ flexDirection: "column", alignItems: "stretch", gap: 8 }}>
