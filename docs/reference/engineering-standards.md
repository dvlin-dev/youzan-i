...
| AI | **@openai/agents**（OpenAI Agents SDK，TS） | `tool()`=工具层、`needsApproval`=HITL、guardrails=注入/越权防御、Agent loop=copilot；经 OpenAI 兼容网关（`OPENAI_BASE_URL`）接模型，`chat_completions` API |
...
- **写操作走 Server Action / typed tool**：内部强制守恒（出库不为负）+ RBAC + 审计 + 生成 `pending` 待复核。
- **双人复核**：`reviewer_id ≠ creator_id` 后端强校验。
- **AI 只能调白名单工具**，输入过 Zod；不写裸 SQL、不直接落库、写操作先出预览经 HITL。
