...
stock_ledger          —— append-only，只有 INSERT，无 UPDATE / DELETE（库内行皆 posted）
  ...
  ts           业务时间
  operator_id  录入人 FK→app_user.id   reviewer_id  复核人 FK→app_user.id（可空）
...
```text
purchase_order: po_no PK, supplier, status, created_by FK→app_user.id, eta, created_at
...
stocktake:       pd_no PK, scope, status(待复核|已过账), snap_ts(账面快照时点),
                 counter(盘点人) FK→app_user.id, created_by FK→app_user.id, counted_at
...
```text
app_user: id, name, email, password_hash, role(warehouse | buyer | admin)
```

所有「谁操作」字段（流水/草稿的 operator_id·reviewer_id、采购单 created_by、盘点单 counter·created_by）一律存 `app_user.id` 并**外键强约束**——每笔留痕都能追溯到真实用户，不再是可乱填、可指向幽灵的自由文本人名；展示时按 id join 取名（`queries.userNames`）。`query_sql` 经**脱敏视图 `app_user_public(id, name, role)`**（无 email / password_hash）join 取操作人姓名，口令表本体对只读角色 DB 层拒读。

### query_audit（AI 只读 SQL 审计表）

```text
query_audit: id PK, ts, actor_id, actor_name(冗余存名，便于用户删除后仍可追溯),
             role, sql(截断 2000 字), outcome(ok|rejected|error|disabled), reason, row_count
```

`query_sql` 每次调用（成功 / 被拒 / 出错 / 停用）都 append 一行（`lib/ai/audit.ts`，落表失败不阻断查询）。独立 append-only 日志，**不授予任何只读角色读取**（被审计者不可读自己的审计），仅运维经主连接 / db:studio 查看。
...
3. **改变库存的动作需经审批入账。** 出入库、红冲、盘点过账走 `待复核 → 已审批入账`：操作先生成待复核单（不直接落库），任意人审批后才入账（单人即可，审批人可与录入人相同），入账经守恒护栏（库存不为负）——对应"8 人并发、改了没记录、错了没人兜"的痛点。这道审批闸同时是 AI 写操作的 human-in-the-loop。
