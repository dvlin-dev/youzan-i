...
每个工具用 `tool()` 定义：`parameters` 是 Zod schema、`execute` 内部做守恒 + 权限校验、写操作产出 `move_draft` 草稿待复核（复核入账经原子守恒护栏）、`needsApproval` 标记需 HITL 的写操作、全程审计。例：

```text
query_stock(filter)                  只读，按角色脱敏（成本价不进响应体）
append_ledger(sku, delta, bizType)   写：守恒校验 + 不为负 + 生成待复核草稿；复核入账经原子守恒护栏
create_po(supplier, lines)           写：落 draft 态，必经采购确认
receive_po(po_no)                    写：仅生成采购到货草稿（待复核）；到货进度/状态在复核入账后才回写
reconcile_attribute(pd_no, sku)      读+算：第 1 层确定性检测器，产出带证据的假设清单（权威分桶/金额）
explain_diff(sku)                    只读：第 2 层 LLM 按需在第 1 层证据上排序解释（降级安全，不改结论）
post_stocktake(pd_no, keys)          写：追加盘盈/盘亏流水（双人复核）
```
