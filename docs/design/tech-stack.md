...
lib/db/setup-readonly.ts 一次性建两个只读角色（仅 GRANT SELECT；仓管走去 cost_price 视图）+ app_user_public 脱敏视图 + 拒读 query_audit
lib/db/seed.ts          演示数据（79 SKU + 9 埋雷盘点；who 字段存 user.id）
lib/actions.ts          类型化工具层（"use server"）：submitMove / reviewDoc / receivePO / createSku / createPO / createStocktake / … RBAC+守恒+审计
...
lib/ai/audit.ts         AI 工具审计留痕落表 query_audit（query_sql 原始 SQL + 发起人 + 结果；落表失败不阻断）
