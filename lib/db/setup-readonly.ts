...
async function main() {
  ...
  const pwFull =
    process.env.READONLY_PW_FULL ?? randomBytes(18).toString("base64url");
  const pwWh =
    process.env.READONLY_PW_WH ?? randomBytes(18).toString("base64url");
  for (const p of [pwFull, pwWh]) {
    if (!/^[A-Za-z0-9_-]+$/.test(p))
      throw new Error("口令含非法字符，请用 base64url 口令");
  }
  ...
  console.log(
    `  · ${WH}  仓管（脱敏视图 sku 无 cost_price + stock_ledger/move_draft）`,
  );
